...
trait Solver extends GameDef {
   ...
   */
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = for((b,m) <- b.legalNeighbors.to(LazyList)) yield (b,m :: history)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
                       explored: Set[Block]): LazyList[(Block, List[Move])] = neighbors.filter(m => !explored.contains(m._1))
  ...
  lazy val solution: List[Move] = pathsToGoal match {
    case LazyList() => Nil
    case _ => pathsToGoal.toList.map(x => x._2).head.reverse
  }
}
